...
class LanguageRepository extends BaseRepository implements LanguageRepositoryInterface
{
    ...
    public function index($obj, $request)
    {
        try {
            ...
            if ($query) {
                $responseData = LanguageResource::collection($query)->response()->getData();
    ...
    // Store: Create a new language
    public function store($obj, $request)
    {
        try {
            $language = $obj::create($request);
            if ($language) {
                return $this->success(new LanguageResource($language), Constants::STORE, Response::HTTP_CREATED, true);
            } else {
    ...
    // Show: Display a specific language
    public function show($obj, $id)
    {
        try {
            $language = $obj::findOrFail($id);
            if ($language) {
                return $this->success(new LanguageResource($language), Constants::SHOW, Response::HTTP_OK, true);
            } else {
    ...
    public function update($obj, $request, $id)
    {
        try {
            ...
            if ($obj) {
                ...
                if ($updated) {
                    return $this->success(new LanguageResource($obj), Constants::UPDATE, Response::HTTP_CREATED, true);
                } else {
    ...
    public function patch($obj, $request)
    {
        try {
            ...
            if ($updated) {
                return $this->success(new LanguageResource($obj), Constants::PATCH, Response::HTTP_CREATED, true);
            } else {
    ...
    public function restore($obj, $id)
    {
        try {
            ...
            if ($data) {
                $data->restore();
                return $this->success(new LanguageResource($data), Constants::RESTORE, Response::HTTP_CREATED, true);
            } else {
