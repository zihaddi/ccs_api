...
use App\Http\Resources\Admin\DynamicHeader\DynamicHeaderResource;
...
class DynamicHeaderRepository extends BaseRepository implements DynamicHeaderRepositoryInterface
{
    use Access;
    use HttpResponses;
    use Helper;
    ...
    public function index($obj, $request)
    {
        try {
            ...
            if ($query) {
                $responseData = DynamicHeaderResource::collection($query)->response()->getData();
    ...
    public function store($obj, $request)
    {
        try {
            ...
            if ($treeEntity) {
                return $this->success(new DynamicHeaderResource($treeEntity), Constants::STORE, Response::HTTP_CREATED, true);
            } else {
    ...
    public function show($obj, $treeEntity)
    {
        try {
            ...
            if ($treeEntity) {
                return $this->success(new DynamicHeaderResource($treeEntity), Constants::SHOW, Response::HTTP_OK, true);
            } else {
        ...
        return new DynamicHeaderResource($treeEntity);
    }
    ...
    public function update($obj, $request)
    {
        try {
            ...
            if ($obj) {
                return $this->success(new DynamicHeaderResource($obj), Constants::UPDATE, Response::HTTP_CREATED, true);
            } else {
    ...
    public function patch($obj, $request)
    {
        try {
            $obj->update($request);
            if ($obj) {
                return $this->success(new DynamicHeaderResource($obj), Constants::PATCH, Response::HTTP_CREATED, true);
            } else {
    ...
    public function restore($obj, $id)
    {
        try {
            ...
            if ($treeEntity) {
                $treeEntity->restore();
                if ($treeEntity) {
                    return $this->success(new DynamicHeaderResource($treeEntity), Constants::RESTORE, Response::HTTP_CREATED, true);
                } else {
            ...
            DB::raw('( SELECT
                    ...
                    WHERE
                        role_id = ' . $profile_id . ')
                               t1'),
            function ($join) {
                $join->on('dynamic_headers.id', '=', 't1.view');
            }
        )
            ->where('pid', 0)
            ->where('status', '=', 1)
                ...
                'children' => function ($q)  use ($profile_id) {
                    $q
                        ->join(
                            ...
                            function ($join) {
                                $join->on('dynamic_headers.id', '=', 't1.view');
                                $join->orOn('dynamic_headers.id', '=', 't1.add');
                                $join->orOn('dynamic_headers.id', '=', 't1.edit');
                                $join->orOn('dynamic_headers.id', '=', 't1.edit_other');
                                $join->orOn('dynamic_headers.id', '=', 't1.delete_other');
                            }
                        );
                }
            ])
    ...
    public function  buildmenu($obj)
    {
        ...
        try {
            ...
            if ($treeentrys) {
                $responseData = DynamicHeaderResource::collection($treeentrys)->response()->getData();
                $responseData = (array)$responseData;
                $responseData['permissions'] = $this->getUserPermissions();
                return $this->success($responseData, Constants::GETALL, Response::HTTP_OK, true);
            } else {
