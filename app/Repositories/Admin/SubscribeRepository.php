...
use App\Http\Resources\Admin\Subscribe\SubscribeResource;
...
class SubscribeRepository extends BaseRepository implements SubscribeRepositoryInterface
{
    ...
    public function index($obj, $request)
    {
        try {
            ...
            if ($query) {
                $responseData = SubscribeResource::collection($query)->response()->getData();
    ...
    // Store: Create a new Subscribe
    public function store($obj, $request)
    {
        try {
            $Subscribe = $obj::create($request);

            if ($Subscribe) {
                return $this->success(new SubscribeResource($Subscribe), Constants::STORE, Response::HTTP_CREATED, true);
            } else {
    ...
    // Show: Display a specific Subscribe
    public function show($obj, $id)
    {
        try {
            $Subscribe = $obj::find($id);
            if ($Subscribe) {
                return $this->success(new SubscribeResource($Subscribe), Constants::SHOW, Response::HTTP_OK, true);
            } else {
    ...
    // Update: Fully update an Subscribe
    public function update($obj, $request, $id)
    {
        try {
            ...
            if ($obj) {
                ...
                if ($updated) {
                    return $this->success(new SubscribeResource($obj), Constants::UPDATE, Response::HTTP_CREATED, true);
                } else {
    ...
    // Patch: Partially update an Subscribe
    public function patch($obj, $request)
    {
        try {
            $updated = $obj->update($request->all());
            if ($updated) {
                return $this->success(new SubscribeResource($obj), Constants::PATCH, Response::HTTP_CREATED, true);
            } else {
    ...
    public function restore($obj, $id)
    {
        try {
            ...
            if ($data) {
                $data->restore();
                return $this->success(new SubscribeResource($data), Constants::RESTORE, Response::HTTP_CREATED, true);
            } else {
