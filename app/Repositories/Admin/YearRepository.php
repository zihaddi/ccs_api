...
class YearRepository extends BaseRepository implements YearRepositoryInterface
{
    ...
    public function index($obj, $request)
    {
        try {
            ...
            if ($query) {
                $responseData = YearResource::collection($query)->response()->getData();
    ...
    {
        try {
            $year = $obj::create($request);
            if ($year) {
                return $this->success(new YearResource($year), Constants::STORE, Response::HTTP_CREATED, true);
            } else {
    ...
    {
        try {
            $year = $obj::with('event_details')->find($id);
            if ($year) {
    ...
    public function update($obj, $request, $id)
    {
        try {
            ...
            if ($obj) {
                ...
                if ($updated) {
                    return $this->success(new YearResource($obj), Constants::UPDATE, Response::HTTP_CREATED, true);
                } else {
    ...
    public function restore($obj, $id)
    {
        try {
            ...
            if ($data) {
                $data->restore();
                return $this->success(new YearResource($data), Constants::RESTORE, Response::HTTP_CREATED, true);
            } else {
