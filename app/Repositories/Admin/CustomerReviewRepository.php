...
use App\Http\Resources\Admin\CustomerReview\CustomerReviewResource;
...
class CustomerReviewRepository extends BaseRepository implements CustomerReviewRepositoryInterface
{
    use Access;
    use HttpResponses;
    use Helper;
    ...
    public function index($obj, $request)
    {
        try {
            ...
            if ($query) {
                $responseData = CustomerReviewResource::collection($query)->response()->getData();
    ...
    // Store: Create a new CustomerReview
    public function store($obj, $request)
    {
        try {
            $CustomerReview = $obj::create($request);
            if ($CustomerReview) {
                return $this->success(new CustomerReviewResource($CustomerReview), Constants::STORE, Response::HTTP_CREATED, true);
            } else {
    ...
    // Update: Fully update an CustomerReview
    public function update($obj, $request, $id)
    {
        try {
            ...
            if ($obj) {
                ...
                if ($updated) {
                    return $this->success(new CustomerReviewResource($obj), Constants::UPDATE, Response::HTTP_CREATED, true);
                } else {
    ...
    // Patch: Partially update an CustomerReview
    public function patch($obj, $request)
    {
        try {
            $updated = $obj->update($request->all());
            if ($updated) {
                return $this->success(new CustomerReviewResource($obj), Constants::PATCH, Response::HTTP_CREATED, true);
            } else {
    ...
    public function restore($obj, $id)
    {
        try {
            ...
            if ($data) {
                $data->restore();
                return $this->success(new CustomerReviewResource($data), Constants::RESTORE, Response::HTTP_CREATED, true);
            } else {
