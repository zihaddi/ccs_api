...
use App\Http\Resources\Admin\SmsTemplate\SmsTemplateResource;
...
class SmsTemplateRepository extends BaseRepository implements SmsTemplateRepositoryInterface
{
    use Access;
    use HttpResponses;
    use Helper;
    ...
    // Index: Retrieve all SMS templates
    public function index($obj, $request)
    {
        try {
            ...
            if ($query) {
                $responseData = SmsTemplateResource::collection($query)->response()->getData();
    ...
    // Store: Create a new SMS template
    public function store($obj, $request)
    {
        try {
            $smsTemplate = $obj::create($request);
            if ($smsTemplate) {
                return $this->success(new SmsTemplateResource($smsTemplate), Constants::STORE, Response::HTTP_CREATED, true);
            } else {
    ...
    // Show: Display a specific SMS template
    public function show($obj, $id)
    {
        try {
            $smsTemplate = $obj::find($id);
            if ($smsTemplate) {
                return $this->success(new SmsTemplateResource($smsTemplate), Constants::SHOW, Response::HTTP_OK, true);
            } else {
    ...
    public function update($obj, $request, $id)
    {
        try {
            ...
            if ($obj) {
                ...
                if ($updated) {
                    return $this->success(new SmsTemplateResource($obj), Constants::UPDATE, Response::HTTP_CREATED, true);
                } else {
    ...
    public function patch($obj, $request)
    {
        try {
            ...
            if ($updated) {
                return $this->success(new SmsTemplateResource($obj), Constants::PATCH, Response::HTTP_CREATED, true);
            } else {
    ...
    public function restore($obj, $id)
    {
        try {
            ...
            if ($data) {
                $data->restore();
                return $this->success(new SmsTemplateResource($data), Constants::RESTORE, Response::HTTP_CREATED, true);
            } else {
