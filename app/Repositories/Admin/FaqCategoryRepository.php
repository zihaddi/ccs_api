...
class FaqCategoryRepository extends BaseRepository implements FaqCategoryRepositoryInterface
{
    ...
    // Index: Retrieve all FAQ categories
    public function index($obj, $request)
    {
        try {
            ...
            if ($query) {
                $responseData = FaqCategoryResource::collection($query)->response()->getData();
    ...
    // Store: Create a new FAQ category
    public function store($obj, $request)
    {
        try {
            $faqCategory = $obj::create($request);
            if ($faqCategory) {
                return $this->success(new FaqCategoryResource($faqCategory), Constants::STORE, Response::HTTP_CREATED, true);
            } else {
    ...
    // Show: Display a specific FAQ category
    public function show($obj, $id)
    {
        try {
            $faqCategory = $obj::with('category')->find($id);
            if ($faqCategory) {
                return $this->success(new FaqCategoryResource($faqCategory), Constants::SHOW, Response::HTTP_OK, true);
            } else {
    ...
    // Update: Fully update an FAQ category
    public function update($obj, $request, $id)
    {
        try {
            ...
            if ($obj) {
                ...
                if ($updated) {
                    return $this->success(new FaqCategoryResource($obj), Constants::UPDATE, Response::HTTP_CREATED, true);
                } else {
    ...
    // Patch: Partially update an FAQ category
    public function patch($obj, $request)
    {
        try {
            $updated = $obj->update($request->all());
            if ($updated) {
                return $this->success(new FaqCategoryResource($obj), Constants::PATCH, Response::HTTP_CREATED, true);
            } else {
    ...
    public function restore($obj, $id)
    {
        try {
            ...
            if ($data) {
                $data->restore();
                return $this->success(new FaqCategoryResource($data), Constants::RESTORE, Response::HTTP_CREATED, true);
            } else {
