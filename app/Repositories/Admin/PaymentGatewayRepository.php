...
class PaymentGatewayRepository extends BaseRepository implements PaymentGatewayRepositoryInterface
{
    ...
    // Index: Retrieve all payment gateways
    public function index($obj, $request)
    {
        try {
            ...
            if ($query) {
                $responseData = PaymentGatewayResource::collection($query)->response()->getData();
    ...
    // Store: Create a new payment gateway
    public function store($obj, $request)
    {
        try {
            $paymentGateway = $obj::create($request);
            if ($paymentGateway) {
                return $this->success(new PaymentGatewayResource($paymentGateway), Constants::STORE, Response::HTTP_CREATED, true);
            } else {
    ...
    // Show: Display a specific payment gateway
    public function show($obj, $id)
    {
        try {
            $paymentGateway = $obj::findOrFail($id);
            if ($paymentGateway) {
                return $this->success(new PaymentGatewayResource($paymentGateway), Constants::SHOW, Response::HTTP_OK, true);
            } else {
    ...
    public function update($obj, $request, $id)
    {
        try {
            ...
            if ($obj) {
                ...
                if ($updated) {
                    return $this->success(new PaymentGatewayResource($obj), Constants::UPDATE, Response::HTTP_CREATED, true);
                } else {
    ...
    public function patch($obj, $request)
    {
        try {
            ...
            if ($updated) {
                return $this->success(new PaymentGatewayResource($obj), Constants::PATCH, Response::HTTP_CREATED, true);
            } else {
    ...
    public function restore($obj, $id)
    {
        try {
            ...
            if ($data) {
                $data->restore();
                return $this->success(new PaymentGatewayResource($data), Constants::RESTORE, Response::HTTP_CREATED, true);
            } else {
