...
use App\Http\Resources\Admin\Gender\GenderResource;
...
class GenderRepository extends BaseRepository implements GenderRepositoryInterface
{
    ...
    public function index($obj, $request)
    {
        try {
            ...
            if ($query) {
                $responseData = GenderResource::collection($query)->response()->getData();
    ...
    // Store: Create a new gender
    public function store($obj, $request)
    {
        try {
            $gender = $obj::create($request);
            if ($gender) {
                return $this->success(new GenderResource($gender), Constants::STORE, Response::HTTP_CREATED, true);
            } else {
    ...
    public function update($obj, $request, $id)
    {
        try {
            ...
            if ($obj) {
                ...
                if ($updated) {
                    return $this->success(new GenderResource($obj), Constants::UPDATE, Response::HTTP_CREATED, true);
                } else {
    ...
    public function patch($obj, $request)
    {
        try {
            ...
            if ($updated) {
                return $this->success(new GenderResource($obj), Constants::PATCH, Response::HTTP_CREATED, true);
            } else {
    ...
    public function restore($obj, $id)
    {
        try {
            ...
            if ($data) {
                $data->restore();
                return $this->success(new GenderResource($data), Constants::RESTORE, Response::HTTP_CREATED, true);
            } else {
