...
use App\Http\Resources\Admin\TrustedBrand\TrustedBrandResource;
...
class TrustedBrandRepository extends BaseRepository implements TrustedBrandRepositoryInterface
{
    ...
    public function index($obj, $request)
    {
        try {
            ...
            if ($query) {
                $responseData = TrustedBrandResource::collection($query)->response()->getData();
    ...
    // Store: Create a new TrustedBrand
    public function store($obj, $request)
    {
        try {
            ...
            if (!empty($request['photo'])) {
                // Save new image
                $request['photo'] = $this->image_target_path . '/' . $data->id . '/' . $this->base64ToImage(
                    $request['photo'],
                    $this->image_target_path . '/' . $data->id
                );
                $data->update(['photo' => $request['photo']]);
            }
            if ($data) {
                return $this->success(new TrustedBrandResource($data), Constants::STORE, Response::HTTP_CREATED, true);
            } else {
    ...
    // Update: Fully update an TrustedBrand
    public function update($obj, $request, $id)
    {
        try {
            ...
            if ($obj) {
                ...
                if ($updated) {
                    return $this->success(new TrustedBrandResource($obj), Constants::UPDATE, Response::HTTP_CREATED, true);
                } else {
    ...
    // Patch: Partially update an TrustedBrand
    public function patch($obj, $request)
    {
        try {
            $updated = $obj->update($request->all());
            if ($updated) {
                return $this->success(new TrustedBrandResource($obj), Constants::PATCH, Response::HTTP_CREATED, true);
            } else {
    ...
    public function restore($obj, $id)
    {
        try {
            ...
            if ($data) {
                $data->restore();
                return $this->success(new TrustedBrandResource($data), Constants::RESTORE, Response::HTTP_CREATED, true);
            } else {
