...
use App\Http\Resources\Cms\PaymentGateway\PaymentGatewayResource;
use Carbon\Carbon;
...
class PaymentGatewayRepository implements PaymentGatewayRepositoryInterface
{
    ...
    public function index($obj, $request)
    {
        try {
            ...
            if ($query) {
                $data = PaymentGatewayResource::collection($query)->response()->getData();
    ...
    // Show by slug: Display a specific payment gateway
    public function showBySlug($obj, $slug)
    {
        try {
            $paymentGateway = $obj::where('slug', $slug)->first();
            if ($paymentGateway) {
                return $this->success(new PaymentGatewayResource($paymentGateway), Constants::SHOW, Response::HTTP_OK, true);
            } else {
                return $this->error(null, Constants::SHOW, Response::HTTP_NOT_FOUND, false);
